...
export function WoodLibraryEditor({ woods, onAdd, onUpdate, onDelete }: Props) {
  return <div className="wood-library-editor">
    ...
    <div className="wood-editor-list">{woods.map(wood => <div className="wood-editor-row" key={wood.id}>
      <div className="wood-editor-main">
      ...
      <input className="wood-vendor" aria-label={`Where to buy ${wood.name}`} title="Where to buy this wood" placeholder="Available at — store, vendor, or SKU" value={wood.availableAt ?? ''} onChange={event => onUpdate(wood.id, { availableAt: event.target.value })}/>
    </div>)}</div>
  </div>
}
