...
export function WoodPatterns() {
  return <defs>
    {species.map(wood => <pattern id={`long-${wood.id}`} key={`long-${wood.id}`} width="26" height="15" patternUnits="userSpaceOnUse">
    ...
    {species.map(wood => <pattern id={`end-${wood.id}`} key={`end-${wood.id}`} width="18" height="18" patternUnits="userSpaceOnUse">
